...
int	is_pushable_to_b(t_element *e, t_all *all)
{
	...
	if (e && !e->window)
	{
		if (all->b->nb_element < 2 || (all->b->nb_element && \
			((e->ideal > all->b->start->ideal - tolerance && e->ideal < all->b->end->ideal + tolerance) || \
...
int	is_within_current_chunk(t_all *all, t_element *e)
{
	...
	if (all->curr_chunk == all->nb_chunk && e->ideal >= chunk_size * (all->curr_chunk - 1))
		return (1);
	if (all->curr_chunk == 1 && e->ideal <= chunk_size)
		return (1);
	if (e->ideal >= chunk_size * (all->curr_chunk -1) && e->ideal <= chunk_size * all->curr_chunk)
		return (1);
	return (0);
}
...
int	is_pushable_to_a(t_element *e, t_all *all)
{
	if (e)
	{
		if (is_within_current_chunk(all, e) && ((e->ideal < all->a->start->ideal && e->ideal > all->a->end->ideal) || \
			(e->ideal > all->a->max && all->a->end->ideal == all->a->max) || (e->ideal < all->a->min && all->a->start->ideal == all->a->min)))
			return (1);
	}
	return (0);
}
...
int	is_pushable_to_a2(t_element *e_from, t_element *e_to, t_all *all)
{
	...
	if (e_to && e_from)
	{
		if (is_within_current_chunk(all, e_from) && ((e_from->ideal < e_to->ideal && e_from->ideal > e_to_prev->ideal) || \
			(e_from->ideal > all->a->max && e_to->ideal == all->a->min) || (e_from->ideal < all->a->min && e_to->ideal == all->a->min)))
			return (1);
	}
	return (0);
}
...
int	is_pushable_to_a(t_element *e, t_all *all)
{
	if (e)
	{
		if (is_within_current_chunk(all, e) && (e->ideal == all->a->start->ideal - 1 || \
			(e->ideal == all->b->max && \
			all->b->max > all->a->max && \
...
int	is_pushable_to_a2(t_element *e_from, t_element *e_to, t_all *all)
{
	if (e_to && e_from)
	{
		if (is_within_current_chunk(all, e_from) && (e_from->ideal == e_to->ideal - 1 || \
